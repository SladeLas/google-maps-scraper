"""Scrape company info off of LinkedIn."""
...
def getchildren(elem):
    """Return all descendant elements for the provided element."""

    return elem.find_elements(By.XPATH, ".//*")


class CompanySummary(object):
    """Lightweight container for LinkedIn company listing snippets."""
...
class Company(Scraper):
    """High-level scraper for LinkedIn company profile pages."""
    ...
    def __init__(
        ...
        """Initialise the scraper and optionally execute immediately."""
    ...
    def __get_text_under_subtitle(self, elem):
        """Return the text found underneath a given subtitle node."""
    ...
    def scrape(self, get_employees=True, close_on_complete=True):
        """Scrape the company page, choosing the flow based on auth state."""
    ...
    def __parse_employee__(self, employee_raw):
        """Parse an employee snippet into a serialisable dict."""
        ...
        except (NoSuchElementException, IndexError) as exc:
            print(exc)
            return None

    def get_employees(self, wait_time=10):
        """Extract the list of employees from the 'People' tab."""
        ...
        try:
            ...
            # No dedicated "see all employees" link; continue anyway.
            pass
        ...
        while is_loaded(results_li_len):
            try:
                ...
                # No next button; stop paginating.
                break
    ...
    def scrape_logged_in(self, get_employees = True, close_on_complete = True):
        """Scrape a company profile when authenticated."""
       ...
       #section ID is no longer needed, we are using class name now.
        ...
        try:
            ...
            for span in spans:
                ...
                if "See all" in txt and "employees on LinkedIn" in txt:
                    self.headcount = int(
                        txt.replace("See all", "").replace("employees on LinkedIn", "").strip()
                        )
        ...
        try:
            ...
            showcase, _ = driver.find_elements(By.CLASS_NAME, "company-list")
            driver.find_element(
            ...
            # Related companies card not present; skip.
            pass
    ...
    def scrape_not_logged_in(
        ...
        """Fallback scraping flow for logged-out sessions."""
        ...
        try:
            driver.find_element(
                By.ID, "view-other-showcase-pages-dialog"
            ).click()
            WebDriverWait(driver, 3).until(
            ...
            for showcase_company in showcase_pages.find_elements(
                By.TAG_NAME, "li"
            ):
            ...
            # Optional showcase dialog not present.
            pass
        ...
        try:
            ...
            for i, affiliated_page in enumerate(
                affiliated_pages.find_elements(
                    By.CLASS_NAME, "affiliated-company-name"
                )
            ):
                if i % 3 == 0:
            ...
            # No affiliated companies carousel.
            pass
