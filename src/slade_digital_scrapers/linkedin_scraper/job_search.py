"""High-level helpers for scraping LinkedIn job boards via Selenium."""
...
class JobSearch(Scraper):
    """Encapsulates site navigation and parsing for LinkedIn's Jobs surface."""
    ...
    def __init__(
        ...
        """Initialize the scraper, optionally running immediately."""
    ...
    def scrape(self, close_on_complete=True, scrape_recommended_jobs=True):
        """Dispatch the correct scraping flow based on authentication state."""

        if self.is_signed_in():
            self.scrape_logged_in(
                _close_on_complete=close_on_complete,
    ...
    def scrape_job_card(self, base_element) -> Job:
        """Extract the metadata from a single job card element."""
    ...
    def scrape_logged_in(self, _close_on_complete=True, scrape_recommended_jobs=True):
        """Pull the different LinkedIn job sections when the user is signed in."""
    ...
    def search(self, search_term: str) -> List[Job]:
        """Execute a keyword search and return the resulting job cards."""

        url = os.path.join(
            self.base_url, "search") + f"?keywords={urllib.parse.quote(search_term)}&refresh=true"
        self.driver.get(url)
